...
atomode ships two pure-geometric cKDTree-based sweeps that act
...
Both live in `src/atomode/_pair_relax.py`, assume orthorhombic
...
- `src/atomode/_pair_relax.py::_bond_relax_sweep` — the
  `bond_relax` kernel.
- `src/atomode/_pair_relax.py::_enforce_hard_core` — the projection.
- `src/atomode/supercell.py::Supercell.bond_relax` and
...
- API: `~atomode.Supercell.bond_relax`,
  `~atomode.Supercell.enforce_hard_core`
