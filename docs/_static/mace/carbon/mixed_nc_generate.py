#!/usr/bin/env python
"""Standalone reproducer: C Mixed sp²/sp³ → orient → MACE+wall → movies.
...
docs/structures/ directory — run from there or edit the path.
    python carbon_mixed_nc_generate.py
"""
import numpy as np
...
BOX = 40.0
RNG_SEED = 2026
KW = {'grain_size': 18.0, 'displacement_sigma': 0.02, 'w_graphite': 0.5, 'w_diamond': 0.5, 'bond_weight': 2.5, 'angle_weight': 1.2, 'repulsion_weight': 2.0, 'hard_core_scale': 0.92, 'nonbond_push_scale': 0.85}
VIZ = {'polyhedra_groups': [{'kind': 'triangles', 'center_symbol': 'C', 'vertex_symbol': 'C', 'bond_length': 1.42, 'bond_length_tol': 0.06, 'angle_tol_deg': 5.0, 'color': (0.2, 0.65, 0.3), 'opacity': 0.55}, {'kind': 'tetrahedra', 'center_symbol': 'C', 'vertex_symbol': 'C', 'bond_length': 1.54, 'bond_length_tol': 0.1, 'angle_tol_deg': 15.0, 'color': (0.25, 0.35, 0.85), 'opacity': 0.45}]}
TITLE = 'C Mixed sp²/sp³'
OUT_PREFIX = "carbon_mixed_nc"
STEPS = 50
