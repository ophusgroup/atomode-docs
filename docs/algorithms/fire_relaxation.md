...
tricor implements a simplified variant (fixed momentum mixing and a
...
3. Species-pair restrictions: pairs with zero
   ...
   and {meth}`CoordinationShellTarget.with_cross_species_bonds_only` /
   {meth}`CoordinationShellTarget.with_bonded_species_pairs` set the
   bond graph explicitly.

Implementation notes (current code path):

- The greedy matching loop is JIT-compiled with **numba**
  (`src/tricor/_shell_relax_numba.py`) and produces bit-equivalent
