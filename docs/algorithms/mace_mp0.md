...
tricor-generated cells on **MACE-MP0**, a machine-learning interatomic
potential.  This page summarises the energy model (the Hamiltonian
being minimised) and the update strategies tricor's pipeline uses on
it.  tricor does not implement MACE; it consumes the published
...
approach unphysically closely.  tricor wraps the MACE calculator with
...
- [FIRE relaxation](fire_relaxation.md) — tricor's built-in
