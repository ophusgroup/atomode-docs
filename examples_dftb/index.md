...
These examples take a atomode-generated SiO₂ supercell and refine it
...
```
1. Voronoi tile          (atomode: cell.generate(num_steps=0))
2. Orientation refine    (atomode: cell.refine_initial_orientations)
3. FIRE shell-relax      (atomode: cell.shell_relax)
...
# atomode stages only — DFTB+ skipped (seconds)
python scripts/regen_dftb_examples.py --skip-dftb
```

Output files per regime live in `docs/figures/dftb/`:
...
- [amorphous](amorphous) — short-range tetrahedral network
- [sro](sro) — short-range-ordered SiO₂
- [nanocrystalline](nanocrystalline) — crystalline interior + amorphous boundary
